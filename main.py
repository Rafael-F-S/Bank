...
def log_in(event):
    ...
    try:
        ...
        if pin_number_var.get() != file_data[1]:
            raise messagebox.showerror("Error", "PIN entered doesn't match")
    ...
    # Catch exception if we couldn't open the file or PIN entered did not match account PIN
        ...
        messagebox.showerror("Error", "Please, pay attention to your details.")
...
def perform_deposit():
    ...
    try:
        ...
        account.deposit(amount_text.get())
        # Deposit funds
        ...
        transaction_text_widget = tk.Text(win, height=10, width=15)
    ...
    # Catch and display exception as a 'showerror' messagebox with a title of 'Transaction Error' and the text of the exception
        ...
        messagebox.showerror("Transaction Error", "Amount unable to be deposited")
...
def perform_withdrawal():
    ...
    try:
        ...
        if float(amount_text.get()) < float(account.balance):
            account.withdraw(amount_text.get())
        # Update the transaction widget with the new transaction by calling account.get_transaction_string()
        # Note: Configure the text widget to be state='normal' first, then delete contents, then instert new
        #       contents, and finally configure back to state='disabled' so it cannot be user edited.
            account.get_transaction_string()
            ...
            text_scrollbar = tk.Scrollbar(
                win, command=transaction_text_widget.yview, orient='vertical')
            v = [[i for i in val] for val in account.transaction_list]
...
def plot_interest_graph():
    ...
    figure.suptitle('Interest over 12 months')
...
def create_login_screen():
    ...
    mainLabel = tk.Label(win, padx=16, pady=16, fg='#33adff',
                         font=('arial', 32), text='FedUni Banking', bg='#4d4d4d')
    mainLabel.grid(columnspan=3)

    # ----- Row 1 -----
    # Acount Number / Pin label here
    accLabel = tk.Label(win, font=('arial', 8), fg='white',
                        text='Account Number / PIN', bg='#4d4d4d')
    ...
    # Button1
    btn1 = tk.Button(win, padx=36, pady=36, fg='white', font=('arial', 20), text='1', bg='#4d4d4d')
    btn1.bind('<Button-1>', handle_pin_button)
    btn1.grid(row=2, column=0, sticky="nsew")
    # Button2
    btn2 = tk.Button(win, fg='white', font=('arial', 20), text='2', bg='#4d4d4d')
    btn2.bind('<Button-1>', handle_pin_button)
    btn2.grid(row=2, column=1, sticky="nsew")
    # Button3
    btn3 = tk.Button(win, fg='white', font=('arial', 20), text='3', bg='#4d4d4d')
    ...
    # Button4
    btn4 = tk.Button(win, padx=36, pady=36, fg='white', font=('arial', 20), text='4', bg='#4d4d4d')
    btn4.bind('<Button-1>', handle_pin_button)
    btn4.grid(row=3, column=0, sticky="nsew")
    # Button5
    btn5 = tk.Button(win, fg='white', font=('arial', 20), text='5', bg='#4d4d4d')
    btn5.bind('<Button-1>', handle_pin_button)
    btn5.grid(row=3, column=1, sticky="nsew")
    # Button6
    btn6 = tk.Button(win, fg='white', font=('arial', 20), text='6', bg='#4d4d4d')
    ...
    # Button7
    btn7 = tk.Button(win, padx=40, pady=40, fg='white', font=('arial', 20), text='7', bg='#4d4d4d')
    btn7.bind('<Button-1>', handle_pin_button)
    btn7.grid(row=4, column=0, sticky="nsew")
    # Button8
    btn8 = tk.Button(win, fg='white', font=('arial', 20), text='8', bg='#4d4d4d')
    btn8.bind('<Button-1>', handle_pin_button)
    btn8.grid(row=4, column=1, sticky="nsew")
    # Button9
    btn9 = tk.Button(win, fg='white', font=('arial', 20), text='9', bg='#4d4d4d')
    ...
    # Button Cancel/clear
    btnClear = tk.Button(win, padx=40, pady=40, fg='white', bg="#ff3333",
                         font=('arial', 12), text='Cancel / Clear')
    btnClear.bind('<Button-1>', clear_pin_entry)
    btnClear.grid(row=5, column=0, sticky="nsew")
    # Button 0 here
    btn0 = tk.Button(win, fg='white', font=('arial', 20), text='0', bg='#4d4d4d')
    ...
    btnLogin = tk.Button(win, fg='white', bg='#009933', font=('arial', 12), text='Log In')
...
def create_account_screen():
    ...
    mainLabel = tk.Label(win, font=('arial', 24), fg='#33adff', bg='#4d4d4d', text='FedUni Banking')
    mainLabel.grid(columnspan=4)

    # ----- Row 1 -----
    # Account number label here
    accountLabel = tk.Label(win, font=8, fg='white', bg='#4d4d4d',
                            text="Acc: " + account.account_number)
    accountLabel.grid(row=1, column=0, sticky="nsew")
    # Balance label here
    balance_label = tk.Label(win, font=8, fg='white', bg='#4d4d4d',
                             text="Balance: $" + balance_var.get())
    balance_label.grid(row=1, column=1, columnspan=2, sticky="nsew")
    # Log out button here
    logout_button = tk.Button(win, fg='white', bg='#4d4d4d', font=8,
                              text="Log Out", command=save_and_log_out)
    logout_button.grid(row=1, column=3,  sticky="nsew")

    # ----- Row 2 -----
    # Amount label here
    amount_label = tk.Label(win, font=12, fg='white', bg='#4d4d4d', text="Amount($)")
    amount_label.grid(row=2, column=0, sticky="nsew")
    # Amount entry here
    amount_entry.grid(row=2, column=1, sticky="nsew")
    # Deposit button here
    deposit_button = tk.Button(win, fg='white', bg='#4d4d4d',
                               text="Deposit", command=perform_deposit)
    deposit_button.grid(row=2, column=2, sticky="nsew")
    # Withdraw button here
    withdraw_button = tk.Button(win, fg='white', bg='#4d4d4d',
                                text="Withdraw", command=perform_withdrawal)
    ...
    text_scrollbar.grid(row=3, column=4)
    transaction_text_widget.config(yscrollcommand=text_scrollbar.set)
